...
import { createIconButton, createTextField, updateTooltip } from "@dionysen/ui";
...
export function createThemeColorField(options: ThemeColorFieldOptions): HTMLElement {
  ...
  const copyBtn = createIconButton({
    label: themeT("settings.theme.copyColor"),
    title: themeT("settings.theme.copyColor"),
  });
  copyBtn.classList.add("theme-editor-icon-btn");
  copyBtn.innerHTML = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>`;

  const dropperBtn = createIconButton({
    label: themeT("settings.theme.eyedropper"),
    title: themeT("settings.theme.eyedropper"),
  });
  ...
  dropperBtn.addEventListener("click", async (e) => {
    ...
    updateTooltip(dropperBtn, themeT("settings.theme.eyedropperHint"));
    let failed = false;
    try {
      ...
      updateTooltip(dropperBtn, themeT("settings.theme.eyedropperFailed"));
      window.setTimeout(() => {
        updateTooltip(dropperBtn, themeT("settings.theme.eyedropper"));
      }, 1600);
      if (wasOpen) setOpen(true);
    } finally {
  ...
  copyBtn.addEventListener("click", async () => {
    try {
      ...
      copyBtn.title = themeT("settings.theme.copied");
      window.setTimeout(() => {
        copyBtn.title = themeT("settings.theme.copyColor");
      }, 1200);
    } catch {
      /* ignore */
    }
