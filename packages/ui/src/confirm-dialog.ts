...
export function promptUnsavedChanges(
  ...
  const message = options.message ?? "Your changes will be lost if you don't save them.";
...
export function promptDiskConflict(
  ...
  const title = options.title ?? "File changed on disk";
  const message =
    options.message ??
    "The file has been modified by another program. Saving will overwrite those changes.";
  const overwriteLabel = options.overwriteLabel ?? "Overwrite";
  const saveAsLabel = options.saveAsLabel ?? "Save As…";
  const cancelLabel = options.cancelLabel ?? "Cancel";
