//! System proxy lookup used only by the updater check.
//!
//! Environment variables win over the Windows registry. macOS and Linux have no
//! extra registry source here; those platforms expose the proxy through env.
...
/// Proxy URL for the updater HTTP client, or `None` when the OS has none.
