...
pub unsafe fn inset_traffic_lights(window: &NSWindow, x: f64, y: f64) {
  ...
  // Patched for Inimark: stock wry uses `height + y` and never touches origin.y, which
  // leaves lights flush-top on recent macOS. Re-applied from drawRect so live resize
  // does not flicker.
